...
function showResult() {
    ...
    const tube_1_predict = tube_1_predict_10s * times * (tube_1_predict_percent / 100);
    const tube_2_predict = tube_2_predict_10s * times * (tube_2_predict_percent / 100);
    
    document.getElementById("output").innerHTML = `
        tube_1_predict_10s: ${tube_1_predict_10s}mL<br>
        tube_2_predict_10s: ${tube_2_predict_10s}mL<br>
        tube_1_predict_percent: ${tube_1_predict_percent}%<br>
        tube_2_predict_percent: ${tube_2_predict_percent}%<br>
        예상 1번 튜브 토출량 : ${tube_1_predict.toFixed(3)}mL<br>
        예상 2번 튜브 토출량 : ${tube_2_predict.toFixed(3)}mL<br>
        예상 1번 튜브 토출량 범위 : ${(tube_1_predict - 0.06).toFixed(3)}mL ~ ${(tube_1_predict + 0.06).toFixed(3)}mL<br>
        예상 2번 튜브 토출량 범위 : ${(tube_2_predict - 0.06).toFixed(3)}mL ~ ${(tube_2_predict + 0.06).toFixed(3)}mL<br>
    `;
}